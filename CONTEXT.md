...
**Settings-block contract.** The `litellm_settings`, `general_settings`, and `router_settings` blocks are enforced as an executable contract by `tests/test_settings_contract.py`. The requirements: `litellm_settings.drop_params` is the boolean `true`; `additional_drop_params` is a list of non-empty strings; `callbacks` is a non-empty list whose every dotted path resolves to an existing Python file at repo root (a callback naming a deleted module would silently no-op at runtime); `general_settings.master_key` uses the `os.environ/` pattern so the key stays out of the committed YAML; and `router_settings.num_retries` is a positive int that matches `litellm_settings.num_retries`. Only the four known top-level keys (`litellm_settings`, `model_list`, `general_settings`, `router_settings`) are allowed — an unknown top-level key fails the test as a drift signal. Fallback wiring is validated separately by `test_model_entry_contract.py` and is not duplicated here.
