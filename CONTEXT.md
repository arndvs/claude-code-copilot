...
**Routing contract.** `tests/test_routing_contract.py` (refs #133) verifies the end-to-end routing contract for every alias discovered from `model_list`: the hyphenated Claude Code name is remapped to the dotted upstream model, primaries carry all four editor headers Copilot validates, fallbacks carry the OpenRouter `api_key`, `response_format`/`thinking` are stripped before the upstream, and `stream: true` is forwarded. It also asserts `router_settings.fallbacks` wires each primary to its `-fallback` and covers the wildcard pass-through. Aliases are discovered from the YAML, so a new model entry is covered automatically. It simulates the forwarding decisions against a mock upstream over a representative inbound Anthropic request — no network, no secrets, no Docker, no LiteLLM import.
