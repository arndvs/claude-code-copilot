...
#   1. Dockerfile accepts --build-arg BUILD_SHA and --build-arg BUILD_TIMESTAMP
#   2. docker build without --build-arg still succeeds (defaults to 'unknown')
#   3. docker build --build-arg BUILD_SHA=abc1234 --build-arg BUILD_TIMESTAMP=2024-01-01T00:00:00Z
...
# ── Test 1: Dockerfile declares ARG BUILD_SHA with default ─────────────────
echo "Test 1: Dockerfile declares ARG BUILD_SHA with sensible default"
if grep -qE '^[[:space:]]*ARG[[:space:]]+BUILD_SHA=' Dockerfile; then
    DEFAULT_SHA=$(grep -E '^[[:space:]]*ARG[[:space:]]+BUILD_SHA=' Dockerfile | head -1 | sed 's/.*BUILD_SHA=//')
    if [ "$DEFAULT_SHA" = "unknown" ]; then
        pass "ARG BUILD_SHA=unknown declared"
    else
        fail "ARG BUILD_SHA default is '$DEFAULT_SHA', expected 'unknown'"
    fi
else
    fail "Dockerfile does not declare ARG BUILD_SHA with a default"
fi
...
# ── Test 3: Dockerfile forwards BUILD_SHA to ENV ────────────────────────────
echo "Test 3: Dockerfile forwards BUILD_SHA ARG to ENV"
if grep -qE '^[[:space:]]*ENV[[:space:]]+BUILD_SHA=' Dockerfile; then
    # Verify it references the ARG (ENV BUILD_SHA=$BUILD_SHA or ENV BUILD_SHA=${BUILD_SHA})
    if grep -qE '^[[:space:]]*ENV[[:space:]]+BUILD_SHA=\$\{?BUILD_SHA\}?' Dockerfile; then
        pass "ENV BUILD_SHA=\$BUILD_SHA declared"
    else
        fail "ENV BUILD_SHA is set but does not reference the ARG variable"
    fi
else
    fail "Dockerfile does not declare ENV BUILD_SHA"
fi
...
if command -v docker >/dev/null 2>&1; then
    if docker build -t copilot-proxy-version-test:defaults . >/dev/null 2>&1; then
        ...
        SHA_VAL=$(docker run --rm copilot-proxy-version-test:defaults printenv BUILD_SHA 2>/dev/null || echo "")
        ...
        if [ "$SHA_VAL" = "unknown" ] && [ "$TS_VAL" = "unknown" ]; then
            ...
            fail "Defaults inside container: BUILD_SHA='$SHA_VAL', BUILD_TIMESTAMP='$TS_VAL' (expected 'unknown')"
        fi
...
if command -v docker >/dev/null 2>&1; then
    ...
    if docker build \
        ...
        SHA_VAL=$(docker run --rm copilot-proxy-version-test:custom printenv BUILD_SHA 2>/dev/null || echo "")
        ...
        if [ "$SHA_VAL" = "$TEST_SHA" ] && [ "$TS_VAL" = "$TEST_TS" ]; then
            pass "Build with --build-arg BUILD_SHA=$TEST_SHA BUILD_TIMESTAMP=$TEST_TS visible in container"
        else
            fail "Values inside container: BUILD_SHA='$SHA_VAL', BUILD_TIMESTAMP='$TS_VAL' (expected '$TEST_SHA', '$TEST_TS')"
        fi
